...
When Clavain interserve mode is enabled (`.claude/clodex-toggle.flag` exists) and `CLAVAIN_DISPATCH_PROFILE=interserve` is set, fast/deep map to `-xhigh` variants in `config/dispatch/tiers.yaml`.
