...
Autonomous software agency — orchestrates the full development lifecycle from problem discovery through shipped code. Runs on Autarch TUI, backed by Intercore kernel (Layer 1) and Interspect profiler. 16 skills, 4 agents, 54 commands, 10 hooks, 1 MCP servers. 31 companion plugins as drivers (Layer 3). Key companions: `interflux` (multi-agent review + research), `interphase` (phase tracking, gates, discovery), `interlock` (multi-agent coordination), `interpeer` (cross-AI review), `intertest` (quality disciplines).
