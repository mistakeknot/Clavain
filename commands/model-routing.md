...
**Instrument notes.** `plan-gauge-lint.py` accepts `--repo-root` before or after the plan path (pilot B's orchestrator reported otherwise; pilot D's validator and a direct test corrected it), silently degrades to emitted-text-only matching without it, and reads edit pairs and verify fences, and since 187ff9d also `Create` blocks; pathspecs and the trailer are outside its gauge, and the Preconditions fence is read as a verify step by the shell heuristic. `profile.py --session <id>` is the lane instrument; the melange/review lane exclusion rule stands. The interspect quarantine (48h by default on every evidence row) means `pattern-f-verdict.sh --list` sees verdicts immediately but interspect's own calibration queries do not; whether verdict rows should bypass it is mk's call (it is the anti-feedback-loop guard).
...
**What shipped (clavain 0.6.304, then 0.6.305 for the doc and the timeout).** `hooks/gauge-gate-executor-spawn.sh` writes one register row per refusal (`--role gate --kind gate --verdict FAIL`, the GAUGE lines as note) before it prints the block decision, bounded to fifteen seconds so a slow register cannot time the hook out (25077ad, 6305a89, and the rule-4 follow-up). `scripts/pattern-f-verdict.sh` pre-flights its context through interspect's sanitizer and stores the note and criterion base64-encoded with `note_enc` when the plain context would be refused; `--list` is one joined line per row, backslashes verbatim; the note budget is 300 (25077ad, 6305a89). `scripts/plan-gauge-lint.py` parses `Create \`path\` with:` blocks into its virtual tree, attributes `.bats` paths, and GAUGE005 sees created files (187ff9d, 6305a89). Beads Sylveste-yibw.12 and .14 closed; .13's sanitizer half closed, its quarantine half open for mk; residual validator findings in Sylveste-yibw.15.
...
|---|---|---|---|---|---|---|---|
| E gate rows + sanitizer (two instances, see confound) | 31 | 121K | 12 | 1 | 88% | 99% | 13 |
| F Create grammar (two instances) | 32 | 101K | 9 | 3 | 81% | 98% | 11 |
| H fix-forward | 16 | 98K | 5 | 3 | 66% | 85% | 7 |
| G live `/execute-plan` | 12 | 56K | 2 | 2 | 47% | 85% | 3 |

Against c60de386's 71–82K per turn and about $3 per pilot: the code pilots ran at 98–121K per message because their briefs were longer (each carried two beads' worth of edits and a dozen VERIFY lines) and E and F each paid for a duplicate instance (confound below). G is the shape at its cheapest: an orchestrator that writes nothing and lints nothing runs at 56K per message and, for the first time, under 50% of cost. That last number is the scale-invariance point made twice now: the share fell because the orchestrator did less, not because anything got cheaper.
