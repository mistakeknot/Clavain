...
### 3a0: Compute Grounding (before recording, so the verdict reaches interspect this run)
...
if [[ -n "$BEAD_ID" ]]; then
    ...
    [[ -n "$TEST_SHA" && "$TEST_SHA" == "$HEAD_SHA" ]] && GROUNDED="test-pass-sha@HEAD"
fi
if [[ -z "$GROUNDED" && -n "${results_path:-}" && -f "${results_path:-/nonexistent}" ]] \
   && grep -q 'CONFORMANCE: PASS' "$results_path" 2>/dev/null \
   && [[ ! "$results_path" -ot "$DIFF_PATH" ]]; then
    GROUNDED="criteria-results"
fi
if [[ -z "$GROUNDED" ]]; then
    mkdir -p .clavain/verdicts
    jq -n --arg h "${HEAD_SHA:-unknown}" \
      '{type:"grounding", status:"NEEDS_VERIFICATION", verification_state:"UNVERIFIABLE", model:"none", findings_count:0, summary:"review verdict has no corroborating signal: no test-pass-sha at HEAD, no fresh criteria-results", head_sha:$h, timestamp:(now|todate)}' \
      > .clavain/verdicts/grounding.json
fi
```
...
### 3b: Ground the Verdict, Then Enforce Gate + Record Phase

Read `{OUTPUT_DIR}/synthesis.md` and present to user (~30-50 lines).

**Grounding check (Sylveste-4b5.5).** A review synthesis is opinion — informed,
multi-agent, and still generated by models reading a diff. "Run after tests
pass" in the Notes is an instruction to an agent, not a signal this gate ever
checked, and self-assessed cleanliness collapses under exactly the workloads
that matter (Kambhampati/Stechly). So a CLEAN synthesis only counts as *clean*
when corroborated by one of two concrete signals that already exist:

- `test-pass-sha` equal to current HEAD — the sprint test phase's durable
  artifact (same freshness rule `landing-a-change` applies: a sha that predates
  HEAD proves nothing about HEAD);
- fresh `criteria-results` with `CONFORMANCE: PASS` from Phase 2b — exit codes
  of the criteria's own check blocks.

Neither present → the verdict is **NEEDS_VERIFICATION** (the 9lp.35
VerificationStep vocabulary: UNVERIFIABLE is not success), the phase does NOT
advance, and the fix is stated: run the project's test suite; on pass, record
`test-pass-sha` and re-enter this phase. Findings-bearing synthesis is
unaffected — this only stops an *uncorroborated clean* from wearing the same
label as a corroborated one.

`$GROUNDED` is in scope from Phase 3a0.

```bash
if [[ -z "$GROUNDED" ]]; then
    echo "quality-gates: NEEDS_VERIFICATION — the review synthesis stands, but no concrete signal corroborates it (test-pass-sha absent or stale vs HEAD; no fresh CONFORMANCE: PASS). Run the test suite; on pass: clavain-cli set-artifact \"$BEAD_ID\" test-pass-sha \"\$(git rev-parse HEAD)\" and re-enter Phase 3b. Not advancing phase." >&2
fi

if [[ -n "$BEAD_ID" ]]; then
    clavain-cli set-artifact "$BEAD_ID" "quality-verdict" "${OUTPUT_DIR}/synthesis.md" 2>/dev/null || true
    if [[ -z "$GROUNDED" ]]; then
        : # NEEDS_VERIFICATION recorded above; no gate, no phase advance.
    elif ! clavain-cli enforce-gate "$BEAD_ID" "shipping" ""; then
        ...
        clavain-cli advance-phase "$BEAD_ID" "shipping" "Quality gates passed (grounded: ${GROUNDED})" ""
    fi
fi
```

No bead in context: the same distinction still applies to what you *report* —
present the synthesis as "clean, corroborated by <signal>" only when a signal
from the list above exists; otherwise say "review clean, NEEDS_VERIFICATION —
no test evidence at HEAD". Run the suite before claiming the second half.
