...
native_fixture_append_audit() {
  ...
  case "$phase" in
    ...
    terminal) DISPATCH_CONTROL_RESULT="${DISPATCH_CONTROL_RESULT:-}" ;;
  ...
  project="$(cd "${WORKDIR:-$PWD}" && pwd -P)" || return 1
  response="$(cd "$(dirname "$database")" && "$ic_bin" "--db=$database" --json route record \
    "--agent=${NAME:-native-fixture}" "--model=${MODEL:-fixture}" --rule=dispatch-profile \
    "--project=$project" "--context=$context")" || return 1
...
native_fixture_handoff_to_python() {
  local packet="$1" result_path="$2" child_pid child_rc old_int old_term
  packet="$(jq -c --argjson parent "$$" '. + {fixture_only:true,eligible:false,parent_pid:$parent}' <<< "$packet")" || return 1
  python3 -c 'import importlib.util,sys; s=importlib.util.spec_from_file_location("dispatch_control",sys.argv[1]); m=importlib.util.module_from_spec(s); s.loader.exec_module(m); m.run_fixture_handoff(0,sys.argv[2])' \
    "$_native_fixture_dir/dispatch_control.py" "$result_path" < <(
    python3 - "$_native_fixture_dir/dispatch_control.py" "$packet" <<'PY'
import importlib.util, json, sys
spec = importlib.util.spec_from_file_location("dispatch_control", sys.argv[1])
module = importlib.util.module_from_spec(spec); spec.loader.exec_module(module)
module.write_framed_handoff(1, module.strict_json(sys.argv[2].encode()))
PY
  ) &
  child_pid=$!
