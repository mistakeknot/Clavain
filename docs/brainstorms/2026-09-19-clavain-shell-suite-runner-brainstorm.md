...
zklw is the correct host on measured capability rather than preference. It carries `bats` 1.13, `bats-support`/`bats-assert`, `ic`, `yq`, `gawk`, `jq` and `interphase`; this Mac lacks `yq` and `gawk`, so 51 of the 57 currently-skipped assertions cannot execute here. **Corrected 2026-09-20:** an earlier survey of mine reported the Mac as lacking the bats assertion helpers. That was wrong — the loop tested four hard-coded paths and never tested `npm root -g`, which on this Mac is `/Users/sma/.npm-global/lib/node_modules` and carries both `bats-support` and `bats-assert`, exactly where `test_helper.bash:14` looks. The skip histogram already refuted the claim: its four reasons are yq, gawk, interphase and the calibration candidate, with no helper reason at all. The real shortfall is `yq` and `gawk`, which account for 51 of the 57 skips, and zklw remains the better host on that basis. zklw also matches the Linux that the last green CI run used.
