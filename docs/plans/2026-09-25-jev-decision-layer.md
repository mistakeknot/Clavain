...
revision: 9
supersedes: revision 8 (commit 60e578f), which superseded revision 7 (commit a136b44), revision 6 (commit 272c90e), revision 5 (commit 59243f9), revision 4 (commit fe0793b), revision 3 (commit 87fd357), revision 2 (commit 938fba3) and revision 1 (commit d16a765)
---
...
```json
{
  ...
  "unsupported_host_capabilities": ["native Skill clavain:using-clavain (Unknown skill); router read as a file"],
  "frontier_required": true
}
```

**Accountable decision (revision 9):** Authored by claude-opus-5-5 (reasoning effort high) through the `planning-opus` frontier fallback of the `frontier-planning` dispatch (primary `planning-astra`, gpt-6-astra, effort xhigh). The fallback cause is Codex HTTP 429 on `planning-astra`, an operational failure, as the coordinator confirmed for revisions 6–8. This session did not see the dispatch log, so the cause is the coordinator's and it will correct it if it differs. The frontier requirement is not downgraded. Clavain installation 0.6.324, policy SHA256 `7209d67e29c4d9e668cb1ecd1d4d931600902cba4031fd35866b8b434b34f79b`, policy source `/home/mk/projects/.clavain-jev/config/routing.yaml`, profile default, classification `foundational-invariants` and `difficult-verification`. The native Skill `clavain:using-clavain` was not in this host's skill list (an unsupported host capability); the router body and `docs/canon/reasoning-routing.md` were read as files from the worktree, so the selected root was not re-verified from a skill location. The review folded in here is same-model and provisional, so the other-frontier review requirement remains open and must be met before R6a, R6b or T5 executes. This JSON is not a usage receipt.

```json
{
  "reasons": ["foundational-invariants", "difficult-verification"],
  "rationale": "Narrow revision 9 of a foundational selector plan: closes two P2s from a provisional same-model review of rev 8 (an R6a test that requires an R6b function; a launch argv no longer bound to the fixed plugin-enable template) and records ten P3s as in-task obligations; the authorization-binding decision is a frontier design choice.",
  "producer": {"model": "claude-opus-5-5", "profile": "planning-opus", "reasoning_effort": "high", "fallback_from": "planning-astra", "fallback_cause": "Codex HTTP 429 (operational), per coordinator"},
  "producer_identity_for_review": "claude-opus-5-5 (author; the coordinator only dispatched)",
  "policy_hash": "7209d67e29c4d9e668cb1ecd1d4d931600902cba4031fd35866b8b434b34f79b",
  "folded_review": {"source": "thr_ay39nh2cpv/jev/rev8-review-samemodel.md", "reviewer_model": "claude-opus-5-5", "kind": "declared same-model adversarial", "status": "PROVISIONAL", "verdict": "NEEDS-FIXES", "findings": {"P1": 0, "P2": 2, "P3": 10}, "routing": "review-astra HTTP 429; review-opus excluded producer_model_conflict; mk ruling mk-3b8z"},
  "review_requirement": "other-frontier, owed on revision 9 before R6a, R6b or T5 executes",
  "unsupported_host_capabilities": ["native Skill clavain:using-clavain not in the host skill list; router read as a file"],
  "frontier_required": true
}
```
...
- `scripts/clavain_selector/adapters/base.py` exports `HostAdapter`, `PointUnreachable`, `Outcome`, `load_matrix`, `authorize_by_policy`, `launch_argv_ok`, `LAUNCH_ARGV_TEMPLATE` (revision 9), `fingerprint_paths`, `FingerprintUnavailable`, `FINGERPRINT_MAX_BYTES`, `FINGERPRINT_MAX_PATHS`
...
class ValidationContext:
    ...
    current_read_set_fingerprints: Mapping[str, str]  # revision 8, P2-10: candidate id -> fingerprint recomputed now from that candidate's read_set_paths
...
- **Order guard (revision 7, G3).** `contract.require_canonical(candidates) -> None` raises `NonCanonicalOrder` unless the sequence is already in canonical order. It never calls `canonical_order` (so patching `canonical_order` cannot disable it): it recomputes the salt from the sequence's ids and checks each adjacent pair with `candidate_sort_key(a, salt=s) <= candidate_sort_key(b, salt=s)`, computing the key only through the id components unless the adjacent ids are equal. It accepts `Candidate` objects or `{id, description}` selector views; for views, whose payloads are not available, adjacent equal ids raise. Outside `contract.py`, package code calls the guard and the order function as module attributes (`contract.require_canonical(...)`, `contract.canonical_order(...)`, after `from clavain_selector import contract`). It never imports them by name, so a test that patches `contract.require_canonical` or `contract.canonical_order` reaches every caller. Inside `contract.py`, calls resolve through the module globals at call time, so the patch reaches them too (revision 8, P2-2). Every serializer calls it on entry: `canonical_request_body`, `egress.admit`, `records.build_record` (before `_candidate_entries` and its `[:16]` cap), `questions.build_battery` and `validated_candidates`. `JevCall` is covered through `admit` and `BatteryMismatch`. A `SelectionRequest` whose `candidates` were replaced after construction therefore cannot be serialized, sent or recorded in a different order; it raises, which the orchestrator maps to `internal_error`.
...
`Outcome` binding (revised in revision 8, P2-4). `base.Outcome` replaces `render_payload: Any` with `render_bytes: bytes | None` and gains `payload_sha256: str | None` and `binding_sha256: str | None`. An emitted outcome is built only by `base.emitted_outcome(chosen, validated, *, mode)`. It sets `render_bytes = contract.payload_bytes(chosen.payload)`, `payload_sha256 = sha256(render_bytes)` and `binding_sha256 = dict(validated.bindings)[chosen.id]`. For `kind == "emitted"`, `Outcome.__post_init__` requires all three to be set and `sha256(render_bytes) == payload_sha256 == binding_sha256`, and raises `ValueError` otherwise (→ `internal_error`, native). `bytes` cannot be mutated, so nothing can change the payload between the check and render. Render emits exactly `render_bytes`, spliced verbatim into the host JSON, and reads no `HostEvent` field. The landed `_shape_like` coercion (`str(payload)`) and its `event.tool_response` read are removed. What each Claude point emits:
- (d) `post_tool_output`: the bytes of `{"hookSpecificOutput":{"hookEventName":"PostToolUse","updatedToolOutput":` + `render_bytes` + `}}`. The preparer produces the payload in the host's shape (a `str` when the tool's output is a string), so no coercion is needed.
- (a) `launch_profile`: the payload is the argv list itself, built by the launch preparer (.8), and render emits `{"argv":` + `render_bytes` + `}`. The binding therefore covers every argv element, not only the id. Revision 9 (P2-2) also holds the argv to the landed adapter's fixed template. `base.LAUNCH_ARGV_TEMPLATE = ("claude", "plugin", "enable", "--scope", "local")` is the prefix the landed `claude_code._render_launch` builds. `base.launch_argv_ok(chosen, validated)` is `True` at every other point; at `launch_profile` it is `True` only when `chosen.payload == [*LAUNCH_ARGV_TEMPLATE, chosen.id]` (a `list` of exactly six `str`). A false result makes the authorization verdict false, so the orchestrator records `unauthorized` (row 12) in every mode, shadow included. `emitted_outcome` re-checks it and raises `ValueError` (→ `internal_error`, native) as a backstop.
- (c) `pre_tool`: emits only the fixed `ask` decision. No payload byte is emitted, so the binding constrains nothing that reaches the host there.
...
2. Paths are processed one at a time, and at most one descriptor is open at any moment. For each path, `lst = base._lstat(resolved)`. `base._lstat = os.lstat` is a module-level seam, looked up at call time. `os.path.realpath` calls `os.lstat` itself, once per path component, so tests count and patch only `base._lstat` (revision 8, P2-6). `ENOENT`/`ENOTDIR` give the entry `[resolved, "missing"]`. Any other `OSError` (`EACCES`, `ELOOP`, `EIO`, …) raises.
3. If `stat.S_ISLNK(lst.st_mode)`, the final component was swapped for a symlink after `realpath`: raise. If `not stat.S_ISREG(lst.st_mode)` (directory, FIFO, socket, character or block device), the path is not opened. A FIFO or device node swapped in after the `lstat` is opened with `O_NONBLOCK`, so a FIFO cannot block, and the swap is caught by the dev/ino check in step 4. A device swap also needs `mknod` or root (revision 8, P3-19). The entry is `[resolved, "not_regular", stat.S_IFMT(lst.st_mode), lst.st_dev, lst.st_ino, lst.st_size, lst.st_mtime_ns]`, so adding or removing a file in a directory (which changes its `st_mtime_ns`, and usually `st_size`) still changes the fingerprint, as the landed code did.
...
7. `st2 = os.fstat(fd)`. The pass is stable when `n == st.st_size` and `(st2.st_size, st2.st_mtime_ns) == (st.st_size, st.st_mtime_ns)`. Otherwise rewind with `os.lseek(fd, 0, SEEK_SET)`, refresh `st = st2`, and repeat steps 6–7 exactly once on the same descriptor. Still unstable: raise. A file that is retried therefore reads at most `(st_size + 1) + (st2_size + 1)` bytes, where `st2_size` is the size `fstat` reported after pass 1. That total is still counted against, and bounded by, the step-5 budget (revision 8, P2-7).
8. Stable: the entry is `[resolved, st.st_dev, st.st_ino, st.st_size, st.st_mtime_ns, content_sha256]`.

The result is the sha256 of `json.dumps(sorted(entries), ensure_ascii=True, separators=(",", ":"))`. Every entry's first element is the unique resolved path, so sorting compares only strings. There is no size cutoff and no `large` literal: a same-size, mtime-preserving edit anywhere in a file within the budget changes the fingerprint. The only markers left are `missing` and `not_regular`, and both carry what makes them comparable (absence, or the `lstat` identity and times). `FingerprintUnavailable` never yields a fingerprint. At pre-eligibility it maps to `stale_before_select` (row 4, `detail: fingerprint_unavailable`). At revalidation it maps to `stale_read_set` (row 12), so `revalidate`'s plain `!=` comparison is only ever applied to two real fingerprints. A preparer that cannot fingerprint a candidate's read set does not offer that candidate, because a `None` fingerprint would silently skip the read-set check.

Where the read-set paths come from (revision 8, P2-10). Each candidate has its own read set:
- `PreparedSet.read_set_paths` holds one `(id, paths)` pair per candidate, in canonical order. `paths` is a tuple of path strings, and an empty tuple means no read set. The preparer's `build` returns it with the candidates, and it is bound into the tag.
- `prepare()` requires that a candidate's `read_set_fingerprint` is `None` exactly when its paths are empty. The preparer computes the fingerprint with `base.fingerprint_paths` when it builds the candidate.
- The orchestrator does the I/O, and `contract` stays pure:
  - At pre-eligibility it recomputes `fingerprint_paths(paths)` for every candidate with a read set. Any `FingerprintUnavailable` gives row 4.
  - It passes the results as `ValidationContext.current_read_set_fingerprints` (id → fingerprint).
  - At row 12 it recomputes only the chosen candidate's entry. `FingerprintUnavailable` gives `stale_read_set`.
- `_is_stale` and `revalidate` compare `c.read_set_fingerprint` with `ctx.current_read_set_fingerprints.get(c.id)`. A missing entry for a candidate that has a fingerprint is stale.

Rejected alternative: a fingerprint callable inside `ValidationContext`. It would move filesystem I/O and exception mapping into `contract.py`, which does no I/O. Residuals (Unknowns): intermediate directory components are not pinned; and a non-regular entry is described by `lstat` metadata only, so a same-size rewrite inside a directory that preserves its mtime is not seen (a directory's contents are its entries, which this does detect), and on a filesystem with coarse mtime granularity an entry added and removed within one tick is not seen either (revision 8).
...
class Preparer:
    ...
    event_fields: frozenset[str]                            # revision 8, P2-3: subset of {"session_id", "tool_name", "tool_input", "tool_response"}
    build: Callable[[HostEvent | None, Path], PreparedInput] # (projected event, project_root) -> task, context, candidates, read_set_paths, sources, task_revision
...
class PreparedSet:
    ...
    read_set_paths: tuple[tuple[str, tuple[str, ...]], ...]  # revision 8, P2-10: (id, paths) in canonical order; () = no read set
    tag: str                              # HMAC-SHA256 under a module-private per-process key

PREPARERS: Mapping[str, Preparer] = MappingProxyType({})  # revision 8, P3-11: read-only view of a static dict literal; .7 ships it empty. Dependents add entries in code, never by importlib or config paths. Tests replace it with monkeypatch.setattr(preparers, "PREPARERS", ...).
...
- `prepare()` looks up `registry.integrations[integration].preparer` in `PREPARERS` (unknown name → `NotPrepared`) and requires `point in preparer.points`. It calls `preparer.build(projected, project_root)`, where `projected` is a new `HostEvent` that has `point` and the fields named in `preparer.event_fields`; every other field is `None` and `raw` is an empty mapping (revision 8, P2-3). An `event_fields` entry outside the four allowed names is `NotPrepared`. Every produced id must be in `preparer.vocabulary(project_root)`. The vocabulary function receives only `project_root`, so no event content can widen it. `prepare()` then canonicalizes and computes `bindings` with `contract.payload_sha256`. A non-JSON-native payload is `NotPrepared`. It checks `read_set_paths` against the candidates (the same ids in canonical order; paths empty exactly when the fingerprint is `None`) and tags the set: `tag = HMAC(key, sha256(canonical JSON of every field except payloads and tag, plus bindings))`. `verify` recomputes the tag and also recomputes every binding from the payloads, so a payload mutated after preparation fails verification as well as authorization.
- Tag encoding (revision 8, P3-13). The canonical JSON is `json.dumps(obj, sort_keys=True, ensure_ascii=True, separators=(",", ":"), allow_nan=False)`, where `Path` → `str(path)`, enums → `.value`, tuples → lists, frozensets → sorted lists, and each candidate → a dict of every field except `payload`.
- Key custody (revision 8, P3-12). The key is `secrets.token_bytes(32)`, generated when `preparers.py` is imported. It lives in the process only: it is never persisted, logged or passed to a child, and it is a different key from `egress`'s. Reloading the module, or importing it under a second path (for example `scripts.clavain_selector` as well as `clavain_selector`), creates a second key. Every set tagged under the other key then fails `verify` → `NotPrepared` → `internal_error`, native. That failure is safe. The package is imported only as `clavain_selector`.
- Preparer contract (revision 8, P2-3). `build` may read only the event fields its preparer declares, and `prepare()` enforces that by projection. Ids come only from the vocabulary. Descriptions come from project state or fixed text, never from event content. Payload bytes are computed by the preparer's own deterministic, validated transform from project state and the declared fields; a declared field is never passed through unvalidated. For example, .10 may declare `tool_response` and emit a reduction of it, and .9 (`library`, no event) declares nothing. Each preparer documents, per declared field, how the payload depends on it. The hostile-event test is a dependent obligation (see What each dependent needs from .7).
...
- Like `AdmittedRequest`, the tag is an accidental-bypass guard, not a security boundary: in-process code can read the key, and any caller of the public `from_operator` can mint a valid set, although it is capped at shadow. Within one process the tag gives no more than an identity registry of minted sets plus the stored bindings. It is kept because it also detects `dataclasses.replace` copies and needs no global registry. It stops a new call path, test or dependent from handing `select()` a hand-built candidate list.
...
- `tests/structural/test_selector_questions.py` pins a golden `{QUESTION_SET_VERSION: questions_sha256(fixed two-candidate battery)}` pair for both `fit_questions` values. The battery is built from `contract.canonical_order` of the fixed candidates. Revision 8 (P3-17) adds two pins: the resulting id order for the two-candidate set, and the canonical id order for a fixed five-id set, both as literals. A change to the salt or the sort key then fails the golden test and forces a version bump. Changing any template, key, type or layout without bumping the version fails that test. Bumping the version updates the golden pair in the same commit.
...
class HostAdapter(Protocol):
    ...
    def authorize(self, chosen: Candidate, validated: ValidatedCandidates,
                  policy: AuthorizationPolicy) -> bool: ...  # body is exactly `return authorize_by_policy(chosen, validated, policy)` (S1, rev 8)
...
`authorize` (revision 6, S1; revised in revision 7). `base.authorize_by_policy(chosen, validated, policy)` implements the rule in Selector contract. `ClaudeCodeAdapter.authorize` and every stub's `authorize` return exactly its result; the landed `event.raw.get("authorized")` and the stubs' `NotImplementedError` are both removed. Revision 8 (P2-8) replaces the revision-7 denylist with an allowlist:
- The body of every adapter's `authorize` is exactly the one statement `return authorize_by_policy(chosen, validated, policy)`. That rules out `self`, module globals, closures, `getattr`/`vars` and helper calls by construction.
- In `authorize_by_policy`, every loaded name must be one of:
  - its three parameters;
  - a name it binds locally (including comprehension targets);
  - the builtins `any`, `dict`, `isinstance`, `len` and `str`;
  - the module `contract`, used only as `contract.payload_sha256`.
  It has no `global`, `nonlocal`, `lambda` or nested `def`.
- Narrowing the verdict later means amending this allowlist through the frontier planner.

Rejected alternative: extending the denylist with each known evasion (`getattr(self, …)`, `vars(self)`, a module global set by `parse_event`, a closure, a helper call). A denylist is always one evasion behind. The `library` point, which has no adapter, calls `authorize_by_policy` directly. `HostEvent.raw` stays for `parse_event` and `acknowledge`. Nothing on the authorization path reads it, and (revision 8) neither does render, which reads no `HostEvent` field. `render` receives the `Outcome`, whose immutable `render_bytes` were checked against the binding when the outcome was built, and it emits those bytes verbatim.
...
- `permute-check --cases <path> [--permutations K] [--seed S] --json` (G3; rewritten in revision 7). It is offline: no network, no credential read and no Jev call. Defaults are K = 8 and S = 1. For each case it builds the identity order of the case's candidate list as written, K permutations from `random.Random(f"{S}:{case_id}")`, and the reversed order, K + 2 orders in all. Revision 6's check permuted only the input to `SelectionRequest`, which canonicalizes on construction, so every consumer saw the same order and a consumer that depended on position could never produce a violation. Revision 7 runs two stages:
  ...
  - **Stage B (consumers).** Build the canonical request once, then bypass construction for each order whose ids differ from `contract.canonical_order(candidates)` (revision 9, P3-4: computed with the harness's private reference, see T9): `req2 = copy.copy(req); object.__setattr__(req2, "candidates", permuted)` (and the same for the case's candidate list and the fake answer's `probabilities` tuple). Revision 8 (P1-1) closes a false positive. An order equal to the canonical one is skipped, because a correct serializer accepts it and would be reported `unguarded`. The written order and its reverse can be canonical, a random 2-candidate order is canonical half the time, and every 1-candidate order is. Cases with fewer than 2 candidates are skipped in stage B. Before feeding an order, the check asserts that it is non-canonical, so no canonical order is ever fed. It counts the orders it fed as `orders_tested_B` for each case. For every case with at least 2 candidates this is ≥ 1, because the written order or its reverse is always non-canonical, so the skip cannot silently empty stage B. Feed the result to every entry of `eval.ORDER_CONSUMERS`, a static tuple of `(name, kind, module, attribute)`. The callable is resolved with `getattr(module, attribute)` when the check runs, so a monkeypatch of the module attribute reaches the check (revision 8, P2-2). `kind = "ranker"` entries must give output identical to the canonical run: `eval.rules_rank(task, candidates)`, `eval.shortlist`, the native arm, `eval.expected_winner` and `jev_client.apply_floors` (fed the fake response with its `probabilities` tuple permuted). `kind = "serializer"` entries must raise `contract.NonCanonicalOrder`: `contract.canonical_request_body`, `egress.admit`, `records.build_record`, `questions.build_battery` and `contract.validated_candidates`. This stage proves that no registered consumer relies on the order it is handed: rankers are invariant, and serializers refuse rather than silently emit a different order.
  ...
  - Output keys: `cases`, `orders_per_case` (K + 2), `orders_tested_B` (revision 8: `{case_id: n}`; 0 exactly for cases with fewer than 2 candidates), `stages` (`["A", "B"]`), `consumers` (the `ORDER_CONSUMERS` names), `violations`, `ties` (list of `{case_id, arm, tied_ids, canonical_winner}`), `seed`. It exits 1 on any violation. A tie is reported, not a violation: rankers break exact score ties, and Jev probabilities within 1e-9 are tied, by canonical order, which is the defined behavior.
  - Coverage of the registry. `ORDER_AGNOSTIC` is a static tuple of `(qualified name, reason)` (revision 9, P3-5: `reason` is an enumerated code, see T9's `test_order_consumers_registered`) for functions that read candidates but provably ignore order (for example `contract.validate_request` and `contract.pre_eligibility`, which return the first failing reason by rule, not by position). A structural test walks the selector package's AST. It requires every function matched by the checker's patterns to be in `ORDER_CONSUMERS`, to call `contract.require_canonical`, or to be in `ORDER_AGNOSTIC`; a new consumer that is none of these fails the test. Revision 8 (P2-9) widens the patterns. A function matches if any of these hold:
    - it reads an attribute named `candidates`, `views`, `bindings`, `read_set_paths` or `probabilities`;
    - it has a parameter with one of those names;
    - it has a parameter annotated with `SelectionRequest`, `ValidatedCandidates`, `PreparedSet` or a `Sequence`/`tuple` of `Candidate`;
    - it calls `getattr` with one of those names as a constant string.
    A second structural test (P2-2) finds no `from … import require_canonical` or `from … import canonical_order` anywhere in the package outside `contract.py`.
  - What the check proves, precisely: for the sampled K + 2 orders of each case, construction is invariant (stage A), and every registered consumer is invariant or refuses non-canonical input (stage B), and every function in the package matched by the checker's patterns is registered, guarded or declared order-agnostic with a reason. A helper that receives a candidate sequence under another name (for example `def f(items)`) and is not itself matched is covered only through its matched caller's registration. It does not prove invariance for unsampled orders, for code outside `scripts/clavain_selector/` (dependents' preparers and adapters must add their own consumers), or anything about Jev's own positional bias, which `score`'s `position_balance` measures instead.
...
- Each preparer returns per-candidate `read_set_paths` (P2-10).
- Each dependent adds a test that runs its registered preparer on a recorded fixture event and asserts that the resulting selector views equal the candidates of one of its sealed eval cases. The eval builds `EVAL_CASE` sets through `from_case`, so it never exercises a real preparer's `build` or vocabulary (P3-18).

Revision 9 adds two more:
- .8's launch preparer produces exactly `[*base.LAUNCH_ARGV_TEMPLATE, id]` as each candidate's payload; any other argv is `unauthorized`. A launch adapter for another host that needs a different command returns to the frontier planner (P2-2).
...
| Selector used as authority | Render never emits allow or `updatedInput`; Claude Code (c) is partial (observe/deny only). `authorize()` reads only the chosen candidate, the validated set with its preparer bindings and the registry policy, a missing policy denies, and a true verdict only lets the selection continue to the host's own gate. Tests prove a payload `authorized` field cannot change a verdict (revision 6, S1) and that stdin cannot supply the integration or the candidates (revision 7, S1). A launch argv, which no host gate covers, must equal the fixed plugin-enable template (revision 9, P2-2). |
| Hook payload spoofs authorization | Removed in revision 6 and closed in revision 7. The landed `ClaudeCodeAdapter.authorize` returned `event.raw.get("authorized")`, a field anyone who can shape the hook JSON controls; the signature no longer receives a `HostEvent`, and an AST allowlist (revision 8) requires every adapter's `authorize` body to be exactly `return authorize_by_policy(chosen, validated, policy)` and limits the names `authorize_by_policy` may load (Risks row updated in revision 9, P3-8). Revision 6 still let stdin supply the candidates, whose payloads drive the applied effect; revision 7 takes the integration from the registry and the candidates from a trusted preparer, binds each payload by hash, and re-checks the hash when the outcome is emitted. Residual: task and context from the hook may steer which authorized candidate is picked. |
...
- Create: `tests/fixtures/selector/order_guard_import.py` (revision 8 negative AST fixture: imports `require_canonical` by name; never imported)
...
- `test_require_canonical` (revision 7): `require_canonical` accepts `canonical_order(xs)` and raises `NonCanonicalOrder` for its reverse and for a swapped adjacent pair; it still raises with `monkeypatch.setattr(contract, "canonical_order", lambda cs: tuple(cs))` (it does not call it); for a duplicate-id list, `require_canonical(canonical_order(xs))` does not raise (one deduplicated salt, revision 8, P3-15); every serializer (`canonical_request_body`, `egress.admit`, `records.build_record`, `questions.build_battery`) raises `NonCanonicalOrder` for a request whose `candidates` were replaced with `object.__setattr__` after construction. `validated_candidates` does not exist until R6b, so its clause is R6b's `test_validated_requires_canonical` (revision 9, P2-1).
...
- `test_payload_sha256_single_source`: `records` has no local `_sha256_payload` implementation and uses `contract.payload_sha256`; `claude_code._hash_payload(x) == contract.payload_sha256(x)` for str, dict, list, nested, `None`, bool and number values. Revision 8 (P3-14): `contract.payload_sha256` and `contract.payload_bytes` raise `ValueError` for a tuple, a set, a dict with a non-`str` key, `float("nan")` and an arbitrary object, and `payload_sha256(x) == sha256(payload_bytes(x))`. The adapter copy's agreement on those values is R6b's.
- `test_order_guard_module_calls` (revision 8, P2-2): an AST walk over `scripts/clavain_selector/` finds no `ImportFrom` that names `require_canonical` or `canonical_order` outside `contract.py`, and every call to either outside `contract.py` is an `Attribute` on the name `contract`; a negative fixture source with `from clavain_selector.contract import require_canonical` fails the same checker. In-task obligation (revision 9, P3-3): outside `contract.py`, every `Load` of `contract.require_canonical` or `contract.canonical_order` must be the `func` of a `Call`, and no `getattr` on `contract` may name either as a string; the negative fixture gains `g = contract.require_canonical`, `def f(x, _g=contract.canonical_order)`, `functools.partial(contract.require_canonical)` and `getattr(contract, "require_canonical")`, and the checker reports each.
- `test_salt_dedup` (revision 8, P3-15): `order_salt(["a", "b", "a"]) == order_salt(["b", "a"])`; `build_record` on a row-2 request with duplicate ids (`fallback_reason="invalid_input"`) does not raise.
...
- `test_questions_golden`: the pinned `{"clavain-qs-1": <sha256>}` pair for the fixed two-candidate battery, for `fit_questions` true and false, plus (revision 8, P3-17) the pinned canonical id order of the two-candidate set and of a fixed five-id set. The hashes and orders are computed once when this test is written and pinned as literals.
...
- Landed tests amended by R6a (revision 8, P2-5). Each amendment only adds the new required keyword, and no assertion is removed or loosened:
  - In `test_selector_records.py`, the helper `_base_kwargs` (L41) gains `fit_questions=True`. It is used by `test_record_shape`, `test_no_forbidden_content`, `test_refused_record_has_no_summaries`, `test_applied_values`, `test_bounds`, `test_permissions`, `test_outcome_join` and `test_unwritable_raises`.
  - `_append_worker` (L160), used by `test_concurrent_append`, calls `build_record` directly and gains `fit_questions=True` too.
  - No landed adapter or egress test changes in R6a.
...
**Files:**
- Modify: `scripts/clavain_selector/contract.py` (`Provenance`, `ValidatedCandidates` with `bindings`/`provenance`, `validated_candidates`, `AuthorizationPolicy`; revision 8: `ValidationContext.current_read_set_fingerprints`, `_is_stale`/`revalidate` read it per id)
- Create: `scripts/clavain_selector/preparers.py` (`PreparedSet` with `read_set_paths`, `Preparer` with `event_fields`, empty read-only `PREPARERS`, event projection, `prepare`, `from_operator`, `from_case`, `verify`, `request_from`, `validated`, `NotPrepared`)
...
- Modify: `scripts/clavain_selector/adapters/base.py` (Protocol signature, `Outcome.render_bytes`/`payload_sha256`/`binding_sha256` and its check, `emitted_outcome`, `authorize_by_policy`, revision 9 `LAUNCH_ARGV_TEMPLATE`/`launch_argv_ok`, `fingerprint_paths` rewrite with the `_lstat` seam, `FingerprintUnavailable`, `FINGERPRINT_MAX_BYTES`, `FINGERPRINT_MAX_PATHS`), `scripts/clavain_selector/adapters/claude_code.py` (`authorize` delegates; render splices `render_bytes` and reads no event field; `_shape_like` deleted; `_hash_payload` drops its `default=str`/`repr` fallbacks and `acknowledge` returns `unknown` when it raises), `scripts/clavain_selector/adapters/stubs.py` (`authorize` delegates)
- Create: `tests/fixtures/selector/authorize_self_state.py` (negative AST fixture: `authorize` variants that read `self._last.raw`, call `getattr(self, "x")`, read a module global set by `parse_event` and call an adapter helper, plus an `authorize_by_policy` variant that reads a module global; never imported)
...
- `test_validated_requires_canonical` (revision 9, P2-1; moved from R6a's `test_require_canonical`): `validated_candidates` raises `NonCanonicalOrder` for a request whose `candidates` were replaced with `object.__setattr__` after construction.
...
- `test_authorize_does_not_read_event` (revision 8 allowlist, P2-8): an `ast` walk over `adapters/*.py` finds that the body of every function named `authorize` is exactly one statement whose `ast.dump` equals that of `return authorize_by_policy(chosen, validated, policy)`, and that `authorize_by_policy` loads only its parameters, its own local names, the builtins `any`, `dict`, `isinstance`, `len` and `str`, and `contract` (only as `contract.payload_sha256`), with no `global`, `nonlocal`, `lambda` or nested `def`. The same checker, run on `tests/fixtures/selector/authorize_self_state.py`, reports a violation for each variant in it (`self._last.raw`, `getattr(self, "x")`, a module global, a helper call, and an `authorize_by_policy` reading a module global), so the test proves the checker can fail.
- `test_outcome_binding` (revision 8, P2-4): `emitted_outcome(chosen, validated, mode="active")` sets `render_bytes == contract.payload_bytes(chosen.payload)` (a `bytes` object), `payload_sha256` and `binding_sha256 == dict(validated.bindings)[chosen.id]`. A hand-built emitted `Outcome` raises `ValueError` if any of the three is missing, if `render_bytes` hashes to something other than `payload_sha256`, or if `payload_sha256` differs from `binding_sha256`. Render output:
  - At (d), the Claude render output contains `render_bytes` verbatim and is byte-identical for two events whose `tool_response` differ (one a str, one a dict).
  - At (a), the rendered `argv` equals the payload list exactly.
  - An AST check finds that `render` and the `_render_*` helpers read no attribute of `event` and pass `event` to no call, and `_shape_like` no longer exists.
- `test_launch_argv_template` (revision 9, P2-2): `base.LAUNCH_ARGV_TEMPLATE` equals the prefix the landed `claude_code._render_launch` built (`["claude", "plugin", "enable", "--scope", "local"]`). At `launch_profile`, `launch_argv_ok` is `True` for `[*LAUNCH_ARGV_TEMPLATE, chosen.id]` and `False` for `["sh", "-c", chosen.id]`, for the template with another id, with a seventh element, with the id missing, and for a tuple or `str` payload; at every other point it is `True`. `emitted_outcome` for a launch candidate whose payload fails the template raises `ValueError`, even though its bytes match the binding.
- `test_hash_payload_agreement` (revision 8, P3-14): `claude_code._hash_payload` raises exactly where `contract.payload_sha256` does. `acknowledge` returns `unknown` for a `next_event` whose `tool_response` is not JSON-native.
- `test_preparers` (revision 7, S1): `prepare` with an unknown preparer name, a point the preparer does not serve, or a `build` that returns an id outside `vocabulary(project_root)` raises `NotPrepared`; the vocabulary function is called with `project_root` only (spy); a valid fixture preparer yields a canonical `PreparedSet` with `provenance == PREPARER` and bindings equal to the payload hashes; `verify` rejects a set rebuilt with `dataclasses.replace` (any field, including one candidate's payload), a set with a copied tag from another set, and a set whose payload `dict` was mutated after preparation; `from_operator` and `from_case` yield `OPERATOR` and `EVAL_CASE` sets that verify. Revision 8 additions:
  - `build` receives a projected event: a spy shows that the undeclared fields are `None` and `raw == {}`. An `event_fields` name outside the allowed four raises `NotPrepared`, and so does a non-JSON-native payload.
  - `verify` rejects a set whose `read_set_paths` were replaced. A candidate whose fingerprint is set but whose paths are empty, or the reverse, raises `NotPrepared`.
  - `PREPARERS["x"] = …` raises `TypeError`.
  - The preparer key is 32 bytes and differs from `egress._EGRESS_KEY`.
  - Two sets prepared from equal inputs carry equal tags (the encoding is deterministic).
- `test_preparer_hostile_event` (revision 8, P2-3; the template each dependent copies for its own preparer): a fixture preparer declares `{"tool_name"}` and derives one payload from `tool_name` by a documented transform. Spoofing `tool_input`, `tool_response`, `raw["authorized"]`, `raw["candidates"]` and `raw["integration"]` leaves the set's ids, descriptions, bindings and `read_set_paths` byte-identical to a clean event's. Changing `tool_name` changes that payload exactly as the transform specifies. In-task obligation (revision 9, P3-7): `prepare()` raises `NotPrepared` when any `read_set_paths` entry, after `os.path.realpath`, gets a rule from `egress._source_rule(path, project_root)` (outside `project_root`, or denylisted such as `~/.ssh/…` or `~/.config/jev/…`), and the preparer contract requires `build` to fingerprint only paths that pass that rule; this test adds a fixture preparer that derives a read-set path from `tool_name` and asserts `NotPrepared` for `../../.ssh/id_x` and for `~/.config/jev/secrets.env`.
- `test_read_set_per_candidate` (revision 8, P2-10): with `a` and `b` carrying different fingerprints, `current_read_set_fingerprints={"a": fa, "b": fb}` gives `pre_eligibility(...) is None` and `revalidate(..., "a", ctx) is None`. Changing only `b`'s entry makes `pre_eligibility` return `stale_before_select`, while `revalidate` for `a` is still `None`. A mapping with no entry for a fingerprinted candidate is stale.
...
- `test_fingerprint_single_open` (revision 8 wording, P2-6): a spy on the `base._lstat` seam (not on `os.lstat`, which `os.path.realpath` calls once per path component) shows exactly one call per distinct resolved path. Spies on `os.open`, `os.stat`, `builtins.open` and `Path.read_bytes` show:
  - at most one `os.open` per distinct path: exactly one per regular file, and none for missing or non-regular paths;
  - zero calls to the others by path;
  - at most one descriptor open at any time;
  - every fd closed (`os.fstat(fd)` raises `EBADF` afterwards).
...
- `test_fingerprint_unavailable_never_equal` (revision 7, P1-2): each of unstable content, `EACCES` from a patched `base._lstat`, `EMFILE` and `EIO` from a patched `os.open`, and a symlink swapped in after `realpath` raises `FingerprintUnavailable` on two consecutive calls; neither call returns a value, so no two results can compare equal; `revalidate` with a fingerprint function that raises maps to `stale_read_set`.
...
- `test_fingerprint_symlink_swap` (revision 8, P2-6): a path that `realpath` resolves, then replaced by a symlink inside a patched `base._lstat` before it calls the real `os.lstat` → `FingerprintUnavailable`. Patching the seam, rather than `os.lstat`, keeps the swap after `realpath` has resolved the path.
- `test_fingerprint_budget`: a sparse file with `st_size` 65 MiB (created with `truncate`) → `FingerprintUnavailable`, no bytes read; a file that grows by 1 MiB per read while being hashed stops at `st_size + 1` bytes per pass and raises as unstable. Its total bytes read are ≤ `(s0 + 1) + (s1 + 1)`, where `s0` and `s1` are the `st_size` values of the first two `os.fstat` results the test's spy records (revision 8, P2-7, matching procedure step 7); 4097 distinct paths → `FingerprintUnavailable` before any `lstat`.
...
- Landed tests amended by R6b (revision 8, P2-5). Each amendment builds its inputs through the new API and keeps or strengthens every assertion; none is loosened:
  - `test_selector_adapters.py::test_render_never_allows` (L126): the emitted outcome is `emitted_outcome(candidate, validated, mode="active")` over a `ValidatedCandidates` for its `cand-a` request (payload `"replacement text"`), instead of `Outcome(kind="emitted", …, render_payload=…)`. The forbidden-snippet loop is unchanged. New assertion: the landed hand-built form, which passes the removed `render_payload=` keyword, now raises `TypeError` (revision 9, P3-2); a hand-built emitted `Outcome` with inconsistent hashes raises `ValueError`, as `test_outcome_binding` asserts.
  - `test_post_tool_render_shape` (L151): the same construction for `cand-shape`. It still asserts that `updatedToolOutput` is a `str` equal to `"replacement text"`, which now holds because the payload is a `str`, not because of coercion. The acknowledge half is unchanged.
  - `test_launch_render_does_not_execute` (L186): `profile-x` gets the payload `["claude", "plugin", "enable", "--scope", "local", "profile-x"]` and an `emitted_outcome`. The assertion `"profile-x" in argv` becomes `argv == payload`, and the `subprocess` patches are kept. Revision 9 (P2-2): a `profile-x` candidate whose payload is `["sh", "-c", "profile-x"]` makes `emitted_outcome` raise `ValueError`, so nothing is rendered.
  - `test_selector_contract.py::test_pre_eligibility` (L68) uses `current_read_set_fingerprints={}`, and `test_revalidate_each_reason` (L77) uses `current_read_set_fingerprints={"a": "fp-1"}`. Every assertion is unchanged, and the latter adds: an empty mapping gives `STALE_READ_SET`.
  - `test_selector_records.py`: `_base_kwargs` and `_append_worker` also gain `provenance=Provenance.OPERATOR`, with no assertion changed.
...
**Step 3:** Implement per the Selector contract "Authorization", `Outcome` binding, fingerprint and read-set-paths paragraphs, the Trusted preparers subsection, the registry validation paragraphs and the Host adapters `authorize` paragraph. Remove `event.raw.get("authorized")` and the stubs' `authorize` `NotImplementedError`.
...
- run: `cd /home/mk/projects/.clavain-jev/tests && uv run pytest structural/ -q -k selector`
  expect: exit 0
- run: `cd /home/mk/projects/.clavain-jev && ! grep -rn "raw.get(\"authorized\"\|\"large\"\|\"unstable\"\|\"unreadable\"\|_shape_like\|render_payload" scripts/clavain_selector/adapters/`
  expect: exit 0
</verify>
...
- `test_payload_mutation_not_emitted` (revision 7, S1; revised in revision 8, P2-4): in active mode, a fake server selects `a`.
  ...
  - With no tampering, the emitted bytes contain `contract.payload_bytes(a.payload)` verbatim, and that is the only payload they contain. The record's `payload_sha256` for `a` is the sha256 of those bytes.
...
- `test_fingerprint_unavailable_maps` (S2): fingerprinting a candidate's `read_set_paths` raises `FingerprintUnavailable` → `stale_before_select` with `detail: fingerprint_unavailable` before the call, and `stale_read_set` when raised only at revalidation.
- `test_per_candidate_read_sets` (revision 8, P2-10): a fixture preparer offers `a` and `b` with disjoint read sets (one real file each). With both fresh there is no stale fallback and Jev is called. After Jev selects `a`, editing only `b`'s file before row 12 does not reject `a`, and editing `a`'s file gives `stale_read_set`. Only the chosen candidate is re-fingerprinted at row 12 (spy).
- `test_launch_argv_unauthorized` (revision 9, P2-2): a fixture launch preparer whose payload for `a` is `["sh", "-c", "a"]`, under an allowing policy, gives `validation.reject_reason == "unauthorized"` (row 12) in shadow and in active mode, and nothing is emitted; the template payload for `a` reaches `shadow_mode` in shadow and `applied: emitted` in active mode.
- `test_emit_rechecks_binding` (revision 9, P3-1, in-task obligation): immediately before render, the orchestrator re-checks `outcome.binding_sha256 == dict(validated.bindings)[chosen.id]`. With `base.emitted_outcome` monkeypatched to return a self-consistent emitted `Outcome` for other bytes (both hashes equal to the sha256 of those bytes), the result is `internal_error` and nothing is emitted.
- `test_select_fingerprint_budget` (revision 9, P3-6, in-task obligation): one `select` reads at most `FINGERPRINT_MAX_BYTES` and `FINGERPRINT_MAX_PATHS` in total across rows 4 and 12, and a file in several candidates' read sets is read once per select. With 16 candidates sharing one 8 MiB file, an `os.read` spy counts at most 8 MiB + 1 bytes read at row 4; with 16 candidates that each add a distinct 5 MiB file to one shared 5 MiB file, the aggregate overruns and the result is `stale_before_select` with `detail: fingerprint_unavailable`.
...
- `test_permutation_invariance` (G3; rewritten in revision 7; revision 8, P1-1 and P2-2): `permute-check --cases <fixture> --permutations 8 --seed 1 --json` exits 0 with:
  - `violations == []`, `orders_per_case == 10` and `stages == ["A", "B"]`;
  - every `ORDER_CONSUMERS` name listed;
  - `orders_tested_B[c] >= 1` for every case with at least 2 candidates, and `== 0` for the 1-candidate case;
  - zero socket attempts;
  - the fixture's tie case listed under `ties` with its canonical winner.

  The fixture's canonical-written 2-candidate case gives no violation: that is the P1-1 regression. A spy wrapped around one serializer (via `monkeypatch.setattr` on its module) records every sequence stage B feeds it and asserts that each is non-canonical.

  Negative runs, each in-process by loading `scripts/selector-eval.py` with `importlib` and calling its `main(argv)`. Every patch is `monkeypatch.setattr(<module>, "<name>", …)` on the defining module, which reaches every caller because callers use module attributes (test_order_guard_module_calls) and `ORDER_CONSUMERS` resolves its callables when the check runs:
  - (i) `eval.rules_rank` replaced with a ranker that returns its input order, and with one that uses a score-only stable sort (ties broken by position, exercised by the fixture's tie case) → stage-B `differs` violations naming `rules_rank`, exit 1.
  - (ii) `contract.require_canonical` patched to a no-op → stage-B `unguarded` violations for every serializer, exit 1.
  - (iii) `contract.canonical_order` patched to the identity → stage-A violations, including `exception` entries where a serializer raised `NonCanonicalOrder`, exit 1, and the check completes without an uncaught exception. In-task obligation (revision 9, P3-4): the harness makes its stage-B skip and non-canonical self-check decisions with a private reference to the real `canonical_order`, bound when `eval.py` is imported (`_REFERENCE_CANONICAL_ORDER`), not with the patched module attribute, and records a failed self-check as a `harness_self_check` violation instead of raising; under (iii) this test also asserts `orders_tested_B[c] >= 1` for every case with at least 2 candidates.
- `test_order_consumers_registered` (revision 7, G3; patterns widened in revision 8, P2-9): an AST walk over `scripts/clavain_selector/` finds every function matched by the checker's patterns (Eval harness, Coverage of the registry), and each is in `ORDER_CONSUMERS`, calls `contract.require_canonical`, or is in `ORDER_AGNOSTIC` with a non-empty reason. The negative fixture source holds one unregistered reader per pattern, and the checker reports each of them:
  - a `.views` read;
  - a `bindings` parameter;
  - a parameter annotated `ValidatedCandidates`;
  - `getattr(req, "candidates")`;
...
stages:
  ...
  - name: "Wave 2b — revision-6/7 amendments"
    tasks:
      ...
      # Revision 8: R6a adds the order-guard negative fixture; R6b's amended landed tests live in the listed test files.
      - {id: task-r6a, title: "Canonical order, request-body single source and question-set identity", files: [scripts/clavain_selector/contract.py, scripts/clavain_selector/questions.py, scripts/clavain_selector/records.py, scripts/clavain_selector/egress.py, schemas/selector-decision-record.v1.schema.json, tests/fixtures/selector/order_guard_import.py, tests/structural/test_selector_contract.py, tests/structural/test_selector_questions.py, tests/structural/test_selector_records.py, tests/structural/test_selector_egress.py, tests/structural/test_selector_adapters.py, scripts/clavain_selector/adapters/claude_code.py], depends: [task-1, task-2, task-4, task-6]}
...
- The eval builds candidate sets with `from_case` (`EVAL_CASE` provenance), so it never measures a real preparer's `build`, vocabulary or event projection; each dependent's preparer-to-case test covers that link (revision 8).
...
```json
{
  "decisions": [
    ...
    "permute-check stage B feeds only non-canonical orders, skips cases with fewer than 2 candidates and reports orders_tested_B; ORDER_CONSUMERS resolves callables at check time and package code calls contract.require_canonical/canonical_order as module attributes (rev 8)",
...
**Open question for mk (criteria change needs `CLAVAIN_RESEAL=1` and your approval).** The reviewer notes that criterion 1 detects skips only for Interstat, so a skipped S1 or S2 test (FIFO, device or symlink setup, `alarm`) would pass silently. The recommendation is to name `test_authorize_ignores_payload`, the `test_fingerprint_*` tests and `test_permutation_invariance` in a criterion, or to add a no-skip check. The author did not edit the criteria. The revision 7 proposed text is superseded: its keyword skip filter could not work, because pytest's `-rA` skip lines carry no test name. The corrected proposal is in the Revision 8 fold-in below. If mk approves, the criterion goes into the Acceptance Criteria section, the handoff's verification line changes to "criteria 1-17", and the seal is regenerated with `CLAVAIN_RESEAL=1`. If mk declines, the tests still run under criterion 1, but they are not gated by name.

### Revision 7 changes

- Frontmatter: `revision: 7`, and `supersedes` names revision 6 (272c90e). The header gains a Revision 7 paragraph, recording the provisional same-model review and its routing (review-astra 429 ×2, review-opus `producer_model_conflict`, mk-3b8z), and a revision-7 accountable-decision block (planning-opus fallback after Codex HTTP 429 ×2, operational, as corrected in a136b44; policy `7209d67e…`; Clavain 0.6.324; review requirement other-frontier still open).
...
### Revision 8 (same-model provisional review of revision 7)

Source: `/home/mk/.bb-machines/autarch.getbb.app/thread-storage/thr_ay39nh2cpv/jev/rev7-review-samemodel.md`, verdict NEEDS-FIXES (P1: 1, P2: 9, P3: 9). This is a declared same-model adversarial review (claude-opus-5-5 reviewing claude-opus-5-5), marked PROVISIONAL. It was routed like the revision-6 review: review-astra returned HTTP 429 twice, and review-opus was excluded as `producer_model_conflict` (mk-3b8z). The review covers `git diff 272c90e a136b44` against the landed selector code.

Before writing a fix, the author checked each claim against the landed code:
- P2-2: every package module imports contract names by name.
- P2-4: `claude_code._shape_like`, `_render_launch` and `base.Outcome`.
- P2-5: the landed adapter, contract and records tests. `_append_worker` is a break the review did not name.
- P2-6: `os.path.realpath` on Python 3.12.3 called `os.lstat` 5 times for a 5-component path.
- P2-10: `contract.ValidationContext` and `revalidate`.

None was rejected. Where a finding needed a design choice, the smallest change that closes it was taken, and the rejected alternative is given in one sentence.

| # | Sev | Disposition |
|---|-----|-------------|
| 1 | P1 | Accepted. Stage B feeds only orders whose ids differ from `contract.canonical_order(candidates)`, skips cases with fewer than 2 candidates, asserts before feeding that each order is non-canonical, and reports `orders_tested_B` per case (≥ 1 for every case with at least 2 candidates, because the written order or its reverse is always non-canonical). The T9 fixture gains a 1-candidate case and a 2-candidate case written in canonical order, and `test_permutation_invariance` asserts the counts and, through a spy, that no canonical order is ever fed. Rejected alternative: accepting a normal return on a canonical order as "guarded". That would loosen the serializer rule, the step the review warns reopens G3. |
| 2 | P2 | Accepted. Package code outside `contract.py` calls `contract.require_canonical`/`contract.canonical_order` as module attributes, `ORDER_CONSUMERS` holds `(name, kind, module, attribute)` resolved at check time, and the new R6a `test_order_guard_module_calls` (with a negative fixture) forbids by-name imports. Rejected alternative: listing every patch site in the negative tests, which a new consumer module would silently escape. |
| 3 | P2 | Accepted. `Preparer.event_fields` declares which of `session_id`, `tool_name`, `tool_input`, `tool_response` `build` may read, and `prepare()` enforces it by passing a projected event (undeclared fields `None`, `raw` empty). The preparer contract states that ids come from the vocabulary, descriptions come from project state, and payloads come from a validated transform of project state and declared fields. `test_preparer_hostile_event` is the template, and each dependent must ship one for its preparer. The Must-Have and the "What authorization does and does not bind" paragraph are reworded to "stdin cannot add a candidate id or bypass a preparer; payload derivation from the event is bounded by the preparer contract". Rejected alternative: a contract stated in prose only, which a first real preparer could break with nothing to catch it. |
| 4 | P2 | Accepted. `Outcome` stores `render_bytes` (immutable canonical bytes), `payload_sha256` and `binding_sha256`, built only by `base.emitted_outcome` from `validated.bindings` and checked in `__post_init__`. Render splices exactly `render_bytes`, and `_shape_like` and its `event.tool_response` read are removed. The launch payload is the argv list, so the binding covers the argv. Pre-tool emits only the fixed `ask`, and that is stated. Rejected alternative: an id-only launch binding, which lets the bytes the host runs differ from the bound bytes. |
| 5 | P2 | Accepted. R6a lists `_base_kwargs` and `_append_worker` (`fit_questions`). R6b lists `test_render_never_allows`, `test_post_tool_render_shape`, `test_launch_render_does_not_execute`, `test_pre_eligibility`, `test_revalidate_each_reason`, and `_base_kwargs`/`_append_worker` (`provenance`). Each entry gives the new construction and states that no assertion is loosened; several are strengthened (`argv == payload`, hand-built emitted outcomes raise). |
| 6 | P2 | Accepted. The post-resolution `lstat` goes through the `base._lstat` seam. `test_fingerprint_single_open` counts only the seam, and `test_fingerprint_symlink_swap` and the `EACCES` case patch it. Rejected alternative: a spy that ignores frames inside `posixpath.realpath`, which depends on stdlib internals. |
| 7 | P2 | Accepted. Procedure step 7 states the bound for a retried file, `(st_size + 1) + (st2_size + 1)`, within the 64 MiB counter, and `test_fingerprint_budget` asserts it using the sizes its `fstat` spy records. |
| 8 | P2 | Accepted. Allowlist: every adapter's `authorize` body is exactly `return authorize_by_policy(chosen, validated, policy)`. `authorize_by_policy` may load only its parameters, its own locals, five named builtins and `contract.payload_sha256`, with no `global`, `nonlocal`, `lambda` or nested `def`. The negative fixture holds one variant per evasion the review listed. The rejected alternative (a longer denylist) is given in Host adapters. |
| 9 | P2 | Accepted. The registration checker also matches `views`, `bindings`, `read_set_paths` and `probabilities` reads and parameters, parameters annotated `SelectionRequest`/`ValidatedCandidates`/`PreparedSet`/`Sequence[Candidate]`, and `getattr` with those names. The claims are reworded to "every function matched by the checker's patterns", and the renamed-helper residual is stated. The negative fixture has one reader per pattern. |
| 10 | P2 | Accepted. `PreparedSet.read_set_paths` (per candidate, bound into the tag) is the path source. `ValidationContext.current_read_set_fingerprints` is a per-id mapping that the orchestrator recomputes: all candidates at row 4, and only the chosen one at row 12. `FingerprintUnavailable` maps as before. New tests: `test_read_set_per_candidate` (R6b) and `test_per_candidate_read_sets` (T7). Rejected alternative: a fingerprint callable in `ValidationContext`, which puts I/O in the pure `contract.py`. |
| 11 | P3 | Accepted. `PREPARERS` is a `MappingProxyType`, and tests replace it with `monkeypatch.setattr`. |
| 12 | P3 | Accepted. The key is `secrets.token_bytes(32)` per process: never persisted, and distinct from egress's key. The second-import-path and reload failure is fail-safe (`NotPrepared` → native) and stated. The "guard, not a boundary" framing is now in the Must-Have too. |
| 13 | P3 | Accepted. The tag encoding is specified: sorted keys, `ensure_ascii`, compact separators, `str(path)`, `enum.value`, tuples as lists, frozensets as sorted lists. |
| 14 | P3 | Accepted. Payloads are JSON-native only, `contract.payload_bytes`/`payload_sha256` raise `ValueError` otherwise (`NotPrepared` at preparation), and the `default=str`/`repr` fallbacks are dropped in both copies. |
| 15 | P3 | Accepted. `order_salt` always deduplicates (`sorted(set(ids))`) and is the only salt computation. `test_salt_dedup` covers a duplicate-id row-2 `build_record`. |
| 16 | P3 | Accepted. `position_balance` is labelled descriptive only, never a gate, with the power caveat. The test asserts that the exit code does not depend on it. |
| 17 | P3 | Accepted. The golden test also pins the canonical id order of the two-candidate set and of a five-id set. |
| 18 | P3 | Accepted. Non-claims record that the eval never runs a real preparer. Each dependent adds a preparer-to-sealed-case test. |
| 19 | P3 | Accepted. The duplicate R6b Step 2 line is removed (the complete one is kept). The Revision 7 changes bullet now records Codex HTTP 429 ×2 (operational), as corrected in a136b44. The L261 wording now says a swapped-in FIFO or device is caught by the dev/ino check. The coarse-mtime residual for non-regular entries is added to the procedure and to Unknowns. |

Acceptance criteria changed in revision 8: none, so no reseal. Criterion 1 still runs the whole selector suite, including the new tests. Criterion 8 still holds, because no schema field is added in revision 8. No new `FallbackReason` exists, because `NotPrepared`, `NonCanonicalOrder` and the `Outcome` check all map to `internal_error`.

**Open question for mk: proposed criterion 17, revised (a criteria change needs your approval and `CLAVAIN_RESEAL=1`; this is a proposal, not an edit).** The revision-7 review made two corrections, and both are applied here:
- (a) Any `^SKIPPED` line in the selector suite fails, as criterion 13 does. This replaces the keyword filter, which could not match: `-rA` skip lines carry no test name.
- (b) It also names the review's additional critical tests. Three revision-8 tests are added as well: `preparer_hostile_event`, `order_guard_module_calls` and `per_candidate_read_sets`.
- (c) Revision 9 (P3-10) adds `read_set_per_candidate`, `salt_dedup` and `fingerprint_single_open`, and fails on `XFAIL` and `XPASS` lines as well as `SKIPPED`, so a critical test marked `xfail(strict=False)` cannot pass silently. The revision-9 tests `validated_requires_canonical`, `launch_argv_template` and `launch_argv_unauthorized` are not in the proposal; mk may add them when approving.

Like criteria 1, 12 and 13, the check uses `PIPESTATUS`, so the check runner must use bash. It turns any skip, xfail or xpass in the selector suite into a failure. A test that must skip on some platform would then need an explicit allowlist, which also needs your approval.

> 17. **The security and determinism tests run and pass by name, and nothing in the selector suite is skipped.**
>
>     ```check
>     cd /home/mk/projects/.clavain-jev/tests && uv run pytest structural/ -q -k selector -rA 2>&1 | tee /tmp/selector-named.txt; test "${PIPESTATUS[0]}" -eq 0 && ! grep -qE '^(SKIPPED|XFAIL|XPASS)' /tmp/selector-named.txt && python3 -c 'import re,sys; s=open(sys.argv[1]).read(); miss=[t for t in ("authorize_ignores_payload","authorize_does_not_read_event","authorize_rule","hook_ignores_stdin_candidates","payload_cannot_authorize","select_requires_prepared","payload_mutation_not_emitted","provenance_limits_mode","outcome_binding","preparers","preparer_hostile_event","fingerprint_unavailable_never_equal","fingerprint_unstable","fingerprint_non_regular","fingerprint_symlink_swap","fingerprint_swap_between_lstat_and_open","fingerprint_budget","fingerprint_single_open","fingerprint_unavailable_maps","per_candidate_read_sets","read_set_per_candidate","require_canonical","salt_dedup","order_guard_module_calls","permutation_invariance","run_records_permutation_check","order_consumers_registered") if not re.search(r"^PASSED \S+::test_"+t+r"\b",s,re.M)]; print("missing:",miss) if miss else None; sys.exit(1 if miss else 0)' /tmp/selector-named.txt
>     ```

If mk approves, the criterion goes into the Acceptance Criteria section, the handoff's verification line changes to "criteria 1-17", and the seal is regenerated with `CLAVAIN_RESEAL=1`. If mk declines, the tests still run under criterion 1, but they are not gated by name and a skip passes silently.
...
- P1-1: stage B skips canonical orders and cases with fewer than 2 candidates, asserts non-canonical input and reports `orders_tested_B`. Covered in the Eval harness, the T9 fixture and `test_permutation_invariance`.
- P2-2: module-attribute calls for the guard and order functions, `ORDER_CONSUMERS` resolved at check time, R6a `test_order_guard_module_calls` and its negative fixture (file list and YAML), and the negative-run patch targets in T9.
- P2-3: `Preparer.event_fields` and event projection, the preparer contract, `test_preparer_hostile_event`, the dependents' hostile-event obligation, and the Must-Have and authorization-scope paragraph reworded.
- P2-4: `Outcome.render_bytes`/`payload_sha256`/`binding_sha256` and `emitted_outcome`; render splices exactly the bound bytes and reads no event field, and `_shape_like` is deleted. The launch payload is the argv, and pre-tool's lack of a payload is stated. `test_outcome_binding` and T7 `test_payload_mutation_not_emitted` are rewritten, the Key link is updated, and the R6b verify greps for `_shape_like`/`render_payload`.
- P2-5: the landed tests amended by R6a and R6b are named, each with its new construction and no loosened assertion.
- P2-6: the `base._lstat` seam (procedure step 2); `test_fingerprint_single_open`, `test_fingerprint_symlink_swap` and the `EACCES` case use it.
- P2-7: the budget bound is `(st_size + 1) + (st2_size + 1)` in procedure step 7 and `test_fingerprint_budget`.
- P2-8: the `authorize` exact-body allowlist and the `authorize_by_policy` name allowlist; the negative fixture has one variant per evasion. Covered in Host adapters, the Protocol comment and `test_authorize_does_not_read_event`.
- P2-9: the widened registration patterns, reworded claims (the Must-Have and "what the check proves"), the stated renamed-helper residual, and one negative reader per pattern.
- P2-10: `PreparedSet.read_set_paths`, the per-id `ValidationContext.current_read_set_fingerprints` and the orchestrator recomputation; R6b `test_read_set_per_candidate`, T7 `test_per_candidate_read_sets`, and the amended contract tests.
- P3-11 to P3-18: a read-only `PREPARERS`; key custody and the reload caveat; the tag encoding; JSON-native payloads with no fallbacks; one deduplicated salt; `position_balance` descriptive only; the golden test pins order; the eval-preparer non-claim and the dependent test.
- P3-19: the duplicate Step 2 is removed, the rev-7 fallback cause bullet is corrected, the FIFO/device wording is fixed, and the coarse-mtime residual is added.
- Criterion 17: the open question is revised with both review corrections and is still a proposal. The Acceptance Criteria section and `.criteria.md`/`.seal` are unchanged, with no reseal.
- Landing and handoff: other-frontier review is owed on revision 8 before R6a, R6b or T5, and five handoff decisions are added.

### Revision 9 (same-model provisional review of revision 8)

Source: `/home/mk/.bb-machines/autarch.getbb.app/thread-storage/thr_ay39nh2cpv/jev/rev8-review-samemodel.md`, verdict NEEDS-FIXES (P1: 0, P2: 2, P3: 10), judged converging. This is a declared same-model adversarial review (claude-opus-5-5 reviewing claude-opus-5-5), marked PROVISIONAL and routed like the revision-7 review. It covers `git diff a136b44 60e578f` against the landed selector code. The coordinator scoped revision 9 as narrow: fix the two P2s, apply each P3 as a one-line text fix or record it as a named in-task obligation, and add no new mechanism beyond the P2-2 check.

Before writing a fix, the author checked each claim against the landed code:
- P2-1: the landed `contract.py` defines no `validated_candidates` or `ValidatedCandidates`.
- P2-2: the landed `claude_code._render_launch` builds `["claude", "plugin", "enable", "--scope", "local"]` and appends the candidate id, so that is the template.
- P3-2: the landed `test_render_never_allows` passes `render_payload=`, which revision 8 removes, so the hand-built form raises `TypeError`.
- P3-7: `egress._source_rule(path, project_root)` already gives `src.outside_project` and `src.denylisted_path`, so the scope check reuses it.

| # | Sev | Disposition |
|---|-----|-------------|
| P2-1 | P2 | Accepted. The `validated_candidates` clause leaves R6a's `test_require_canonical` and becomes R6b's `test_validated_requires_canonical`, so R6a's verify step can pass as written. The Order guard paragraph still lists `validated_candidates` as a guarded serializer, which is true once R6b lands. |
| P2-2 | P2 | Accepted. `base.LAUNCH_ARGV_TEMPLATE` is the landed template, and `base.launch_argv_ok(chosen, validated)` requires a launch payload to equal `[*LAUNCH_ARGV_TEMPLATE, chosen.id]`. The orchestrator folds it into `ValidationContext.authorized`, so anything else is `unauthorized` (row 12) in every mode, and `emitted_outcome` re-checks it as a backstop. The check is outside `authorize_by_policy`, so the revision-8 AST allowlist is unchanged. Tests: R6b `test_launch_argv_template`, the amended landed `test_launch_render_does_not_execute`, and T7 `test_launch_argv_unauthorized`. The Constraint (L149), the launch bullet (L272) and the rejected alternative (L275) now agree, and the Risks row and the .8 obligations say the same. Rejected alternative: an id-only launch payload, which would again let the bytes the host runs differ from the bound bytes. |
| P3-1 | P3 | In-task obligation (T7): the orchestrator re-checks `binding_sha256` against `validated.bindings` immediately before render; closed by `test_emit_rechecks_binding`. |
| P3-2 | P3 | Text fix: the R6b landed-test amendment now expects `TypeError` for the landed `render_payload=` form and `ValueError` for inconsistent hashes. |
| P3-3 | P3 | In-task obligation (R6a): `test_order_guard_module_calls` also forbids any non-call `Load` of the two attributes and string `getattr` on `contract`, with four alias variants in the negative fixture. |
| P3-4 | P3 | In-task obligation (T9): a private `_REFERENCE_CANONICAL_ORDER` drives the stage-B skip and self-check, and a failed self-check is a `harness_self_check` violation; negative (iii) of `test_permutation_invariance` asserts `orders_tested_B >= 1`. The Eval harness stage-B text points to it. |
| P3-5 | P3 | In-task obligation (T9): `ORDER_AGNOSTIC` reasons are enumerated codes, and `guarded_upstream` targets must be registered or guarded; closed by `test_order_consumers_registered` with two new negative entries. |
| P3-6 | P3 | In-task obligation (T7): a per-select aggregate byte and path budget across rows 4 and 12, with each shared file read once; closed by `test_select_fingerprint_budget`. T7 may add an optional `fingerprint_paths` keyword, so `base.py` joins T7's file list and the YAML. |
| P3-7 | P3 | In-task obligation (R6b): `prepare()` refuses read-set paths that `egress._source_rule` flags, and `build` fingerprints only paths that pass; closed by the extended `test_preparer_hostile_event` and carried into the dependents' obligations. |
| P3-8 | P3 | Text fix: the "Hook payload spoofs authorization" Risks row describes the revision-8 exact-body allowlist. |
| P3-9 | P3 | Text fix: the Authorization paragraph pins the order after Jev returns (row-12 fingerprint recompute, then `authorize` and `launch_argv_ok`, then `revalidate`), and `test_payload_mutation_not_emitted` names the hook: the chosen candidate's row-12 `fingerprint` call. |
| P3-10 | P3 | Proposal updated: criterion 17 adds `read_set_per_candidate`, `salt_dedup` and `fingerprint_single_open`, and fails on `XFAIL`/`XPASS` lines. It is still a proposal for mk. |

Not adopted: the review's section 3 note on a parent directory swapped after `realpath`. The reviewer rated it a note, not a finding, and the intermediate-directory residual is already in Unknowns.

Acceptance criteria changed in revision 9: none, so no reseal. The Acceptance Criteria section still hashes to `.seal`. No new `FallbackReason` exists: a failed launch template gives the existing `unauthorized`, and the backstop and the render-time binding recheck give `internal_error`.

### Revision 9 changes

- Frontmatter: `revision: 9`, and `supersedes` names revision 8 (60e578f). The Authorship line and a Revision 9 header sentence are updated, and a revision-9 accountable-decision block is added: author claude-opus-5-5 via the planning-opus fallback; fallback cause Codex HTTP 429 on planning-astra (operational), per the coordinator; the folded review is same-model and PROVISIONAL; the other-frontier review is owed on revision 9 before R6a, R6b or T5; the native Skill was unavailable.
- P2-1: R6a `test_require_canonical` drops the `validated_candidates` clause, and R6b gains `test_validated_requires_canonical`.
- P2-2: `LAUNCH_ARGV_TEMPLATE` and `launch_argv_ok` (Artifacts, R6b files), composed into `ValidationContext.authorized`, with the `emitted_outcome` backstop. The Constraint, launch bullet, rejected alternative, Risks row and .8 obligation are reconciled. Tests: R6b `test_launch_argv_template`, the landed launch-test amendment, and T7 `test_launch_argv_unauthorized`.
- P3-1: T7 `test_emit_rechecks_binding`.
- P3-2: `TypeError` in the R6b landed-test amendment.
- P3-3: R6a `test_order_guard_module_calls` alias obligation.
- P3-4: T9 private reference and `harness_self_check`, and the stage-B pointer.
- P3-5: T9 enumerated `ORDER_AGNOSTIC` reasons, and the registry pointer.
- P3-6: T7 `test_select_fingerprint_budget`, and `base.py` in the T7 file list and YAML.
- P3-7: the R6b `prepare()` scope obligation in `test_preparer_hostile_event`, and the dependents' obligation.
- P3-8: the Risks row text.
- P3-9: the pinned post-Jev order and the named hook in `test_payload_mutation_not_emitted`.
- P3-10 and criterion 17: three names and the `XFAIL`/`XPASS` rule added to the proposal. The Acceptance Criteria section and `.criteria.md`/`.seal` are unchanged.
- Landing and handoff: the other-frontier review is owed on revision 9, and the criterion 17 pointer names the revision 9 fold-in.
