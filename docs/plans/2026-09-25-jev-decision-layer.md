...
revision: 8
supersedes: revision 7 (commit a136b44), which superseded revision 6 (commit 272c90e), revision 5 (commit 59243f9), revision 4 (commit fe0793b), revision 3 (commit 87fd357), revision 2 (commit 938fba3) and revision 1 (commit d16a765)
---
...
- (a) `launch_profile`: the payload is the argv list itself, built by the launch preparer (.8), and render emits `{"argv":` + `render_bytes` + `}`. The binding therefore covers every argv element, not only the id.
...
Rejected alternative: an "id-only" binding for launch. Render would then append the id to an adapter-owned argv, and the bytes the host runs would differ from the bytes that were bound.
...
- Modify: `scripts/clavain_selector/adapters/base.py` (Protocol signature, `Outcome.render_bytes`/`payload_sha256`/`binding_sha256` and its check, `emitted_outcome`, `authorize_by_policy`, `fingerprint_paths` rewrite with the `_lstat` seam, `FingerprintUnavailable`, `FINGERPRINT_MAX_BYTES`, `FINGERPRINT_MAX_PATHS`), `scripts/clavain_selector/adapters/claude_code.py` (`authorize` delegates; render splices `render_bytes` and reads no event field; `_shape_like` deleted; `_hash_payload` drops its `default=str`/`repr` fallbacks and `acknowledge` returns `unknown` when it raises), `scripts/clavain_selector/adapters/stubs.py` (`authorize` delegates)
...
- Landed tests amended by R6b (revision 8, P2-5). Each amendment builds its inputs through the new API and keeps or strengthens every assertion; none is loosened:
  ...
  - `test_launch_render_does_not_execute` (L186): `profile-x` gets the payload `["claude", "plugin", "enable", "--scope", "local", "profile-x"]` and an `emitted_outcome`. The assertion `"profile-x" in argv` becomes `argv == payload`, and the `subprocess` patches are kept.
...
- `test_payload_mutation_not_emitted` (revision 7, S1; revised in revision 8, P2-4): in active mode, a fake server selects `a`.
  - A fake adapter that mutates `a.payload` between Jev's return and row 12 → `unauthorized` (row 12), and nothing is emitted.
...
- `test_permutation_invariance` (G3; rewritten in revision 7; revision 8, P1-1 and P2-2): `permute-check --cases <fixture> --permutations 8 --seed 1 --json` exits 0 with:
  ...
  - (iii) `contract.canonical_order` patched to the identity → stage-A violations, including `exception` entries where a serializer raised `NonCanonicalOrder`, exit 1, and the check completes without an uncaught exception.
...
Before implementation, this plan needs an independent other-frontier plan review (bead notes on mk-42j9.7). If Codex still returns 429, use an adversarial Opus review declared same-model and provisional, and file a capacity-recheck bead. Pass `--producer-identity` from this plan's actual author receipt (held by the coordinator). Revision 1 was reviewed by claude-fable-5-1 (NEEDS-FIXES); revisions 2–5 answered every finding. Revision 6 was reviewed only by a declared same-model adversarial review (claude-opus-5-5, PROVISIONAL, NEEDS-FIXES; review-astra HTTP 429 twice, review-opus excluded as `producer_model_conflict`, fallback per mk's ruling mk-3b8z), which revision 7 answers. A provisional capacity-substitute review never closes the other-frontier requirement, so before R6a, R6b or T5 executes, revision 7 still needs an other-frontier review (not claude-opus-5-5, and not self-review), with `--producer-identity` from the coordinator's receipt for this revision, and the capacity-recheck bead for the provisional review stays open until it runs. Revision 8 answers a second declared same-model review (of revision 7; claude-opus-5-5, PROVISIONAL, NEEDS-FIXES, same routing), which again cannot close the requirement. The other-frontier review is therefore owed on revision 8, and none of R6a, R6b or T5 may execute before it passes. mk also needs to answer the open acceptance-criteria question (criterion 17), last updated in the revision 8 fold-in. Then execute with `clavain:executing-plans`, at most 3 workers in parallel following the waves above, and finish with T13.
